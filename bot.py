...
def extract_tags(entry, soup):
    """Извлекает теги новости из RSS или со страницы."""
    tags = []
    # 1. Из RSS entry.tags
    if 'tags' in entry:
        for tag in entry.tags:
            ...
            if term and term not in tags:
                tags.append(term)
    # 2. Из meta og:tag или article-tags на странице
    if soup:
        ...
        if not tags:
            ...
            for a in tag_links:
                ...
                if t and t not in tags:
                    tags.append(t)
    # Оставляем не более 5 тегов, обрезаем слишком длинные
    clean_tags = []
    for t in tags:
        t = t.strip().replace(' ', '_')
        if len(t) > 30:
...
def format_news_body(text):
    """Форматирует текст с выделением названий в кавычках и разбивкой на абзацы."""
    if not text:
        return ""
    text = re.sub(r'\n(?!\n)', ' ', text)
    text = re.sub(r'\n{3,}', '\n\n', text)
...
def build_post_html(title, body, tags, date_str, video_url=None, is_youtube=False):
    """Собирает итоговый HTML-текст поста с тегами и датой."""
...
def send_post_with_keyboard(title, body, tags, date_str, link, video_url=None, is_youtube=False):
    """Отправляет пост с inline-кнопками «Читать полностью» и «Смотреть видео»."""
    # Подготавливаем текст
    message_text = build_post_html(title, body, tags, date_str, video_url, is_youtube)
    ...
    # Обычное сообщение или фото
    # Для фото подпись ограничена 1024 символа, поэтому обрежем message_text
    # Но так как у нас есть кнопки, можно отправить фото с короткой подписью,
    # а полный текст оставить для отдельного сообщения? Для простоты отправим фото с короткой подписью.
    image_url = fetch_image_url(entry)  # но у нас нет entry здесь, поэтому image_url передаём отдельно
    # Вместо этого упростим: отправляем текст с кнопками, если нет прямого видео.
    # Если есть картинка, отправляем фото с подписью (обрезанной) и кнопками.
    # Чтобы не усложнять, сделаем так: если есть картинка, отправляем фото + кнопки, а полный текст в подписи не более 1000 символов.
    if image_url:
        try:
            r = requests.head(image_url, timeout=5)
            if r.status_code == 200:
                # Обрежем message_text до 1000 символов
